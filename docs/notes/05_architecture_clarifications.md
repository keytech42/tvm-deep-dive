...
- **Temporal vs. Physical Isolation**: It is a common misconception that separate un-fused operations run on "physically distant" execution units. In reality, multiple kernels execute on the *same* physical GPU Streaming Multiprocessors (SMs). The separation is **temporal**: one kernel must completely finish before the next begins. This temporal isolation is what forces the first kernel to write its intermediate results out to VRAM, and the next kernel to read them back.
- **Memory Directionality**: Hardware does not build data upwards. The source of truth is always the massive, slow **VRAM (Global Memory)**. Data is pulled (loaded/fetched) *down* the hierarchy: VRAM (Global Memory) → L2 Cache → L1 Cache (Shared Memory) → Registers. *Note: VRAM is never referred to as "Shared Memory" in GPU architecture; "Shared Memory" strictly refers to the ultra-fast, on-chip L1 scratchpad.*
- **The True Benefit of Sequential Fusion**: `FuseTIR` initially places the three independent loops (`Conv`, `Add`, `ReLU`) **sequentially** within the same physical function boundary (`PrimFunc`); it does not immediately interleave them. However, by existing in the same physical function, these loops bypass the temporal isolation boundary. They can now pass intermediate data to each other via `T.sblock_alloc_buffer` (which maps to L1 Cache/Shared Memory or Registers) rather than flushing data all the way back up to VRAM. This elimination of VRAM I/O is the true power of fusion.
